...
def squad_dataset(squad_datashape: DataShape) -> Dataset:
    ...
    data_path = PROJECT_ROOT / "tests" / "data" / "squad_date_val.parquet"
    ...
    # Limit to 10 samples for fast test execution
    df = df.head(10)
...
def test_execute_perplexity_on_squad(
    ...
    # Verify results
    assert len(measures) > 0, "Perplexity metric returned no measures"
    assert len(measures) == len(squad_dataset.data), (
        f"Expected {len(squad_dataset.data)} measures, got {len(measures)}"
    )
